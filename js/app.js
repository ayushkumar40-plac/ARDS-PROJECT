      ...
      <div class="printable-report bg-slate-900/90 text-slate-100 p-8 rounded-2xl border border-slate-800 shadow-2xl max-w-4xl mx-auto space-y-6">
        ...
        <div class="pt-6 border-t border-slate-800 grid grid-cols-2 gap-8 text-xs text-slate-400">
          ...
          <div>
            ...
            <div class="text-[10px] text-slate-500 mt-1">Telemetry Reference Validation Complete</div>
          </div>
        </div>
      </div>
    `;
...
// ========================================================
// STEP 2: STANDALONE LOCAL TELEMETRY ENGINE & SIMULATOR
// ========================================================

// Helper function to handle telemetry stream actions locally (no MQTT/hardware dependency)
function sendHardwareCommand(actionName, payloadValue = null) {
  console.log(`[LOCAL ENGINE] Executing action: ${actionName}`, payloadValue);
  ...
  if (feed) {
    ...
    line.textContent = `[${time}] ⚡ ENGINE ACTION: ${actionName} ${payloadValue ? JSON.stringify(payloadValue) : ''}`;
  ...
  if (stamp) {
    stamp.textContent = `Last active: ${time}`;
  }

  // Handle specific action behaviors locally
  if (actionName === 'START_STREAM') {
    if (window.ardsApp && typeof window.ardsApp.startSensorSimulator === 'function') {
      window.ardsApp.startSensorSimulator();
    }
  } else if (actionName === 'PAUSE_STREAM') {
    if (window.ardsApp && typeof window.ardsApp.stopSensorSimulator === 'function') {
      window.ardsApp.stopSensorSimulator();
    }
  } else if (actionName === 'RESET_BASELINE') {
    if (window.dataStore && typeof window.dataStore.resetSampleData === 'function') {
      window.dataStore.resetSampleData();
      if (window.ardsApp && typeof window.ardsApp.renderAll === 'function') {
        window.ardsApp.renderAll();
      }
    }
  }
}

// Expose on window for inline onclick attributes
